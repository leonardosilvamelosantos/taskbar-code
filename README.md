...
![Demonstração do Task Bar Hero Code em funcionamento](docs/demo.gif)

Um widget flutuante e minúsculo que fica **sempre visível por cima de tudo**
no seu Windows, mostrando em tempo real o que cada sessão do
[Claude Code](https://claude.com/claude-code) está fazendo — pulso animado de
estado, nome do terminal, subagentes ativos, uso de contexto e do rate limit
de 5h. Roda direto no seu PC (não é uma extensão de navegador nem depende de
nenhum serviço externo) e é **100% livre**: arraste para qualquer lugar da
tela e redimensione do jeitinho que quiser — ele lembra a posição e o
tamanho da próxima vez que abrir.
...
script não servir no seu ambiente, veja **`INSTALL.pt-BR.md`** — documenta o
...
`CONTRIBUTING.pt-BR.md` para o guia completo (convenções de encoding, como
testar mudanças no instalador, etc.).
